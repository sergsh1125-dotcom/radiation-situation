...
def get_custom_marker_html(value_text, date_text):
    ...
    <div style="position: relative; display: flex; align-items: center; width: 220px;">
        ...
        <div style="
            ...
            line-height: 1.2;
            white-space: nowrap;
            text-shadow: 1px 1px 2px white;"> <div>{value_text}</div>
...
with col_gui:
    ...
    if not st.session_state.data.empty:
        ...
        # (Логіка підготовки m_html аналогічна візуалізації нижче)
...
with col_map:
    ...
    if st.session_state.data.empty:
        ...
        start_lat, start_lon, zoom = (df_clean.lat.mean(), df_clean.lon.mean(), 9) if not df_clean.empty else (49.0, 31.0, 6)

    # Створення карти без стандартних тайлів (додамо їх вручну для контролю)
    m = folium.Map(location=[start_lat, start_lon], zoom_start=zoom, tiles=None, control_scale=True)

    # --- ДОДАВАННЯ ТИПІВ КАРТ ---
    folium.TileLayer(
    ...
    # Контроль шарів (тут будуть і типи карт, і дати)
    folium.LayerControl(collapsed=False).add_to(m)

    st_folium(m, width="100%", height=750, key="multi_layer_rad_map")
